...
const Prayers = (() => {
  ...
  // Mission ids — Chat 2 inserted these with the 'daily_' prefix.
  ...
  // Theological correction (per Chat 1 dispatch): the prayer set flips
  // from pre-Chrismation to full AFTER session 00.6 ('The Day I Was Sealed')
  // has been marked complete by Nolan. Symbol of Faith is confessed AFTER
  // the seal of Chrismation, not before.
  //
  // Implementation: query session_progress for a completed_at on session 00.6.
  // If the row exists with completed_at IS NOT NULL → 'full' set.
  // Otherwise → 'pre_chrismation' set.
  //
  // Year 1+ sessions implicitly imply Chrismation has occurred — if Nolan
  // is in Year 1, we trust the sequence and return 'full' without a query.
  async function variantForState(state) {
    if (!state || !state.current_session_id) return 'pre_chrismation';
    const id = state.current_session_id;

    // Year 1+ sessions are post-Chrismation by definition.
    if (!id.startsWith('00.')) return 'full';

    // Topic 00 sessions before 00.6: pre-Chrismation, no DB query needed.
    const n = parseInt(id.slice(3), 10);
    if (n < 6) return 'pre_chrismation';

    // From 00.6 onward, check whether 00.6 has actually been completed.
    if (!sb || !profileId) return 'pre_chrismation';
    try {
      const { data, error } = await sb
        ...
        .eq('session_id', '00.6')
        .maybeSingle();
      if (error) return 'pre_chrismation';
      return data && data.completed_at ? 'full' : 'pre_chrismation';
    } catch (e) {
      return 'pre_chrismation';
    }
  }
