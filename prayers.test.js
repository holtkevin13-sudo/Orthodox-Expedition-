...
(async function run() {
  ...
  // variantForState is now async (queries session_progress for completion).
  // Without a Supabase client (passed null in init), it falls through to
  // pre_chrismation for any 00.X with X>=6, which is the safe default.
  assert('null state → pre_chrismation',
    (await Prayers.variantForState(null)) === 'pre_chrismation');
  assert('Topic 00 session 00.1 → pre_chrismation',
    (await Prayers.variantForState({ current_session_id: '00.1' })) === 'pre_chrismation');
  assert('Topic 00 session 00.5 → pre_chrismation',
    (await Prayers.variantForState({ current_session_id: '00.5' })) === 'pre_chrismation');
  assert('Topic 00 session 00.6 (no completion) → pre_chrismation (correct: not yet sealed)',
    (await Prayers.variantForState({ current_session_id: '00.6' })) === 'pre_chrismation');
  assert('Topic 00 session 00.10 (no completion of 00.6 in DB) → pre_chrismation in test env',
    (await Prayers.variantForState({ current_session_id: '00.10' })) === 'pre_chrismation');
  assert('Year 1 session 1.1 → full (always post-Chrismation)',
    (await Prayers.variantForState({ current_session_id: '1.1' })) === 'full');
  assert('Year 3 session 3.20 → full',
    (await Prayers.variantForState({ current_session_id: '3.20' })) === 'full');
