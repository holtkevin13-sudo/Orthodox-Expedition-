...
const EmailUtils = {
  ...
  // ── 3. PRIZE REDEEMED ───────────────────────────────────────
  // Repair W (May 7 2026): redemptions are AUTO-APPLIED at the DB layer
  // (no status column on `redemptions`). This email is a NOTIFICATION,
  // not an approval request. Copy reframed accordingly: parents see what
  // already happened so they can hand over the physical prize.
  //
  // Two call signatures supported:
  //   • Legacy: { parentEmail, childName, prizeName, ... }
  //         -> single recipient, no name personalization
  //   • Repair W: { parents: [{name, email}, ...], childName, prizeName, lifetimeCoins, ... }
  //         -> sends one email per parent with "Hi [Parent]" greeting
  // Returns true if at least one delivery succeeded.
  async prizeRedeemed(opts){
    const {
      childName, prizeName, prizeEmoji,
      coinsSpent, coinRate, remainingCoins, lifetimeCoins,
    } = opts;

    // Normalize recipient list — accept both legacy and new shapes
    let parents = [];
    if (Array.isArray(opts.parents) && opts.parents.length){
      parents = opts.parents.filter(p => p && p.email);
    } else if (opts.parentEmail){
      parents = [{ name: 'Parent', email: opts.parentEmail }];
    }
    if (!parents.length){
      console.error('prizeRedeemed: no recipients');
      return false;
    }
    ...
    const results = await Promise.all(parents.map(p => this.send({
      to: p.email,
      subject: `${prizeEmoji||'🎁'} ${childName} redeemed ${prizeName}`,
      html: `
        <div style="font-family:Georgia,serif;max-width:520px;margin:0 auto;background:#fdf8ee;border:1px solid #c9922a;border-radius:12px;overflow:hidden;">
          <div style="background:linear-gradient(135deg,#3d1f08,#5c3010);padding:1.5rem;text-align:center;">
            <div style="font-size:1.8rem;color:#f0c96e;margin-bottom:0.25rem;">☩</div>
            <div style="font-family:Georgia,serif;font-size:1.1rem;color:#f0c96e;letter-spacing:0.1em;">The Orthodox Expedition</div>
            <div style="font-size:0.75rem;color:rgba(240,201,110,0.6);letter-spacing:0.2em;text-transform:uppercase;margin-top:0.25rem;">Bazaar Redemption</div>
          </div>
          <div style="padding:1.5rem;">
            <p style="font-size:1rem;color:#3d1f08;margin-bottom:1rem;">Hi ${p.name||'Parent'},</p>
            <p style="font-size:0.95rem;color:rgba(61,31,8,0.78);line-height:1.6;margin-bottom:1.25rem;">
              <strong>${childName}</strong> just redeemed a prize from the Grand Bazaar for ${coinsSpent.toLocaleString()} Saint Coins.
            </p>
            ...
            <table style="width:100%;border-collapse:collapse;margin-bottom:1.25rem;">
              ...
              ${typeof lifetimeCoins === 'number' ? `
              <tr style="border-bottom:1px solid rgba(201,146,42,0.15);">
                <td style="padding:0.4rem 0;font-size:0.8rem;color:rgba(61,31,8,0.5);">Lifetime earned</td>
                <td style="padding:0.4rem 0;font-size:0.85rem;color:#3d1f08;text-align:right;">${lifetimeCoins.toLocaleString()} coins</td>
              </tr>` : ''}
              <tr>
                <td style="padding:0.4rem 0;font-size:0.8rem;color:rgba(61,31,8,0.5);">Current balance</td>
            ...
            <p style="font-size:0.92rem;color:rgba(61,31,8,0.65);line-height:1.6;margin-bottom:1.25rem;font-style:italic;">
              The coins have already been deducted from ${childName}'s balance. When you have a moment, please hand over the prize.
            </p>
            <a href="https://holtkevin13-sudo.github.io/Orthodox-Expedition-/${(p.role==='parent') ? 'parent.html' : 'admin.html'}"
               style="display:block;text-align:center;padding:0.875rem;background:linear-gradient(135deg,#c9922a,#ffd700);border-radius:8px;color:#1a0f00;font-weight:bold;text-decoration:none;font-size:0.9rem;letter-spacing:0.05em;">
              View Their Progress ✦
            </a>
          </div>
